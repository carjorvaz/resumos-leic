...
type CalendarType = 'g' | 'j';
type ResultType = 'weekday' | 'easter';

const CalendarCalculator = () => {
  const [day, setDay] = useState('0');
  const [month, setMonth] = useState('0');
  const [year, setYear] = useState('0');
  const [calendarType, setCalendarType] = useState<CalendarType>('g'); // 'g' or 'j'
  const [resultType, setResultType] = useState<ResultType>('weekday'); // 'weekday' or 'easter'
  const [expressions, setExpressions] = useState<string[]>([]);

  const onChangeNumbers =
    <T extends string>(setFn: Dispatch<SetStateAction<T>>) =>
    (event: ChangeEvent<HTMLInputElement> | ChangeEvent<HTMLDivElement>) =>
      setFn((event.target as HTMLInputElement).value as T);

  const katexStrings = expressions.map((expression) =>
    katex.renderToString(expression, { displayMode: true })
  );

  const isLeapYear = (year: number, calendar: CalendarType) => {
  ...
  const getN = (
    y: number,
    { expressions, calendar }: { expressions: string[]; calendar: CalendarType }
  ) => {
