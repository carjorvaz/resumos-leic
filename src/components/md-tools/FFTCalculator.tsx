...
const FFTCalculator = () => {
  ...
  const onChangeNumbers =
    (i: number, setFn: Dispatch<SetStateAction<string[]>>) =>
  ...
  const mulComplex = (c1: Complex, c2: Complex): Complex => {
    // (a + bi) * (c + di)
    // ac + adi + bci - bd
    return {
  ...
  const sumComplex = (c1: Complex, c2: Complex): Complex => {
    return { real: c1.real + c2.real, imag: c1.imag + c2.imag };
  };
  const complementComplex = (c: Complex): Complex => {
    return { real: c.real, imag: -c.imag };
  };
  const multiplyStep1 = ([v1, v2]: Complex[]): Complex[] => {
    const minusOne: Complex = { real: -1, imag: 0 };
    return [sumComplex(v1, v2), sumComplex(v1, mulComplex(minusOne, v2))];
  };
  const multiplyStep2 = ([v1, v2]: Complex[]): Complex[] => {
    const imagOne: Complex = { real: 0, imag: 1 };
    return [v1, mulComplex(imagOne, v2)];
  };
  const sumStep3 = ([v1, v2]: Complex[], [v3, v4]: Complex[]): Complex[] => {
    const minusOne: Complex = { real: -1, imag: 0 };
    return [
      sumComplex(v1, v3),
      sumComplex(v2, v4),
  ...
  const calculate = () => {
    ...
    const pol1 = pol1Input.map((v) => ({ real: parseInt(v, 10), imag: 0 }));
    const pol2 = pol2Input.map((v) => ({ real: parseInt(v, 10), imag: 0 }));
    const pol1Rev = rev2Fn(pol1);
    const pol2Rev = rev2Fn(pol2);
    const rev2 = `
      \\begin{array}{r l l}
    ...
    const genF = (pol: Complex[]): { result: Complex[]; text: string } => {
