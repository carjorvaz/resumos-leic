...
const TCRTableCalculator = () => {
  ...
  const onChangeNumbers = (i: number, j: number) => (event: ChangeEvent<HTMLInputElement>) =>
    setInput((array) =>
  ...
  const hasSolution = (m: number[], a: number[]): boolean => {
  ...
  const calculateC = (mi: number[]): number[] => {
    // factorize all numbers
    const factorCounts = mi
      .map((m) => {
      ...
      .map((m) =>
        m.reduce((acc: Record<string, number>, v) => {
          if (!acc[v]) acc[v] = 0;
          acc[v]++;
          return acc;
        }, {})
      );
    const lcmFactors = factorCounts.reduce((acc: Record<string, number>, factors) => {
    ...
    const ci = [...factorCounts]
      ...
      .map((c) => Object.entries(c).reduce((acc, [k, v]) => acc * Number(k) ** v, 1));
    return ci;
  };

  const getNTilde = (n: number, c: number): number => {
    // n =_c 1
    // nx - cy = 1
    const remainder = [n, c];
